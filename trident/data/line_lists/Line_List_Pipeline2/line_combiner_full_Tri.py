...
for ion in shared_ions_mortcash:
  ...
  if (len(c_data)==1 and len(m_data)==1):
    ...
    saved_c_data, saved_m_data=c_data, m_data
    print(ion, 'both 1')
  elif (len(c_data)==1):
    lambda_m, lambda_c=np.float64(m_data[:, 0]), np.float64([c_data[:, 0]])
    #print(ion, 'c_data 1')
  ...
  for i in range(len(c_data)):         #for each entry in this ion in Cashman, find nearest Morton line
    ...
    if min(deltas)<d_lambda_threshold:          #Check if close enough
      ...
      #If there is no match, the line is ostensibly not in Morton-->add data from cashman to special dictionary as well as add to our main dictionary
      incashnotmort[ion].append(c_data[i])   
      data=np.concatenate((data, c_data[i].reshape((1,6))))
...
#Now, add in any Morton ions not covered by Cashman, and then ions not covered by Morton
for ion in exclusive_ions_m:
  combined_dict[ion]=morton_dict[ion]
  
for ion in exclusive_ions_cash:
  combined_dict[ion]=cashman_dict[ion]
...
pickle.dump(reduced_dict, open('mortcash_dict_new_full_Tri', 'wb'))
...
datafile='MortCash_full_Tri.txt'
data=np.column_stack([np.array(column) for column in columns])
np.savetxt(datafile, data, fmt='%s')

#Now we add Trident data; either lines only present in Trident, or lines present in Morton/Cashman that are missing both gamma and A values or f values. Lines that are missing gamma but not A have already had A values substituted for gamma, and the gamma values listed in Trident are not used for these.
#Lines exclusive to Cashman have f values, but are missing gamma and A; these will use the Trident gamma values
...
for ion in shared_ions_tri:
  ...
  for tri_line in tri_lines:
    ...
    if min(deltas)<d_lambda_threshold:
      ...
      #  print(ion, tri_lambda, combo_index, 'combo index longer than 1')  
      #  continue
      #else:
      ...
      if (combo_lines[combo_index][1]=='' and combo_lines[combo_index][2]==''):
        ...
        combo_lines[combo_index][2], combo_lines[combo_index][-1]=tri_line[2], combo_lines[combo_index][-1]+'Tri'
...
for ion, data in reduced_dict2.items():
  for line in data:
    ...
    if line[4]=='':
      line[4]='N/A'
pickle.dump(reduced_dict2, open('mortcashtri_dict_new_full_Tri', 'wb'))
...
columns=[ion_column, lambda_column, gamma_column, f_column, elow_column, source_column]
datafile='MortCashTri_full_Tri.txt'
data=np.column_stack([np.array(column) for column in columns])
np.savetxt(datafile, data, fmt='%s')
...
for ion in line_dictionary.keys():
  for line in data:
    if ion in line:
      line_dictionary[ion].append(line[1:])  
  line_dictionary[ion]=np.array(line_dictionary[ion])

mortcashtri_og=line_dictionary
total_lines1, total_lines2=0, 0
total_lines_set1, total_lines_set2=0, 0
for ion1, data1 in mortcashtri_og.items():
  lambda_list1=data1[:, 0]
  lambda_set1=set(list(lambda_list1))
  data2=reduced_dict2[ion1]
  lambda_list2=data2[:, 0]
  lambda_set2=set(list(lambda_list2))
  print(ion1)
  print(len(lambda_list1), len(lambda_set1), len(lambda_list2), len(lambda_set2))
  if len(lambda_list1)!=len(lambda_set1):
    print('og mortcashtri issue', ion1)
  if len(lambda_list2)!=len(lambda_set2):
    print('issue with new list', ion1)
  total_lines1+=len(list(lambda_list1))
  total_lines2+=len(list(lambda_list2))
  total_lines_set1+=len(set(list(lambda_list1)))
  total_lines_set2+=len(set(list(lambda_list2)))
